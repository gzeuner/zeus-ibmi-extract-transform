...
class ManifestPathFormatterTest {
  ...
  void outputFilePathDisplay_shouldReturnRelativePathWhenInsideOutputDirectory() {
    ...
    assertEquals(
        "nested/data.jsonl", ManifestPathFormatter.outputFilePathDisplay(outputDir, outputFile));
  }
  ...
  void querySourceDisplay_shouldRedactAbsolutePathToFileName() {
    assertEquals(
        "customers.sql", ManifestPathFormatter.querySourceDisplay("/home/dev/sql/customers.sql"));
  }
  ...
  void querySourceDisplay_shouldKeepRelativePathReadable() {
    assertEquals(
        ...
        ManifestPathFormatter.querySourceDisplay("queries\\customers.sql"));
  }
}
