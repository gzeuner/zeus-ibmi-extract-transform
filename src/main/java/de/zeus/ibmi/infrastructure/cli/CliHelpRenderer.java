...
public final class CliHelpRenderer {
  ...
  public String render(String toolName, String toolVersion) {
    ...
    help.append("  java -jar ")
        .append(toolName)
        .append(".jar --config config/example.application.properties\n");
    help.append("  java -jar ")
        .append(toolName)
        .append(".jar --config config/example.application.properties --execute\n");
    help.append("  java -jar ")
        .append(toolName)
        .append(
    ...
    help.append("  java -jar ")
        .append(toolName)
        .append(
            ".jar --config config/example.application.properties --db-driver org.h2.Driver --db-url \"jdbc:h2:mem:demo_readonly;MODE=DB2;DB_CLOSE_DELAY=-1\" --query \"SELECT 1 AS ID\" --execute\n\n");
    help.append("  java -jar ")
        .append(toolName)
        .append(
            ".jar --config config/example.application.properties --query-file queries/customers.sql --execute\n\n");
    help.append("General:\n");
    ...
    help.append(
        "  --db-password-env <ENV_VAR>     Resolve password from named environment variable\n");
    help.append(
        "  --allow-empty-password          Allow empty db.password (for local/offline scenarios)\n\n");
    help.append("Query:\n");
    ...
    help.append(
        "  --dry-run                       Force dry-run mode even if --execute is present\n\n");
    help.append("Notes:\n");
    help.append("  - Duplicate CLI options are allowed; the last value wins.\n");
    help.append(
        "  - Prefer ENV-based secrets; use --db-password only consciously for local use.\n");
    help.append(
        "  - Exit codes: 0 success, 1 general, 2 config/args, 3 guard, 4 jdbc, 5 output, 6 manifest.\n");
