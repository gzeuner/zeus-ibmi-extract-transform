...
public final class ReadOnlyQueryGuard {
    ...
    private static final Pattern FORBIDDEN_SQL_KEYWORDS = Pattern.compile(
            "\\b(INSERT|UPDATE|DELETE|MERGE|DROP|ALTER|CREATE|TRUNCATE|CALL)\\b",
            Pattern.CASE_INSENSITIVE);
    private static final Pattern FORBIDDEN_COMMAND_HINTS = Pattern.compile(
    ...
    public String validateOrNormalize(String query) {
        ...
        if (forbiddenSql.find()) {
            throw new QueryGuardException("Forbidden SQL keyword detected: " + forbiddenSql.group(1).toUpperCase(Locale.ROOT));
        }
        Matcher forbiddenCommands = FORBIDDEN_COMMAND_HINTS.matcher(tokenSafe);
        if (forbiddenCommands.find()) {
            throw new QueryGuardException("Potential command execution pattern detected: "
